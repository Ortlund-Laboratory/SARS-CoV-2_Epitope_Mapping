...
# set to your folder
path <- "/home/adkeith@Eu.Emory.Edu/DMS_Workflow/nucleocapsid/Covid_Update_Correlogram/cleanup_correlogram/remove_antimyc_sites"
...
### NEED TO REMOVE VARIANTS WITH STICKINESS TO ANTI-MYC ANTIBODY
### NEED TO REMOVE:
### Sites 2-4, variants V,L,I,M,F,Y,W
# In combined_num, this corresponds to lines:
# Site 2::: 24,27,29,30,36,37,38
# Site 3::: 42,45,47,48,55,56,57
# Site 4::: 62,65,67,68,74,75,76
### Site 5, variants C,S,T,V,L,I,M,F,Y,W
# Site 5::: 78,81,83,85,86,91,92,93,94,95
### Sites 6-14, variants V,L,I,M,F,Y,W
# Site 6::: 100,103,105,106,112,113,114
# Site 7::: 119,122,124,125,131,132,133
# Site 8::: 138,141,143,144,150,151,152
# Site 9::: 157,160,162,163,169,170,171
# Site 10::: 176,179,181,182,188,189,190
# Site 11::: 195,198,200,201,207,208,209
# Site 12::: 213,216,218,219,226,227,228
# Site 13::: 233,236,238,239,245,246,247
# Site 14::: 252,255,257,258,264,265,266
### Sites 18-30, variants V,L,I,M,F,Y,W
# Site 18::: 328,330,332,333,340,341,342
# Site 19::: 347,349,351,352,359,360,361
# Site 20::: 366,369,371,372,378,379,380
# Site 21::: 385,388,390,391,397,398,399
# Site 22::: 403,406,408,409,416,417,418
# Site 23::: 423,426,428,429,435,436,437
# Site 24::: 442,445,447,448,454,455,456
# Site 25::: 461,463,465,466,473,474,475
# Site 26::: 480,483,485,486,492,493,494
# Site 27::: 499,502,504,505,511,512,513
# Site 28::: 518,521,523,524,530,531,532
# Site 29::: 537,540,542,543,549,550,551
# Site 30::: 556,558,560,561,568,569,570
### Sites 32-35, variants D,E,V,L,I,M,F,Y,W
# Site 32::: 592,593,594,597,599,600,606,607,608
# Site 33::: 611,612,613,616,618,619,625,626,627
# Site 34::: 630,631,632,634,636,637,644,645,646
# Site 35::: 648,649,650,653,655,656,663,664,665
### Sites 36-41, all variants
# Sites 36-41 (together)::: 666:779

### NEED TO REMOVE SITES 51-53, 71-73, 84-87, 110-116, 130 and 132
### THESE SITES DISRUPT RBD STABILITY
### In combined_num, this corresponds to lines 951-1007, 1331-1387, 1578-1653, 
### 2072-2204, 2452-2470 and 2490-2508.

remove_idx <- c(24,27,29,30,36,37,38,42,45,47,48,55,56,57,
                62,65,67,68,74,75,76,78,81,83,85,86,91,92,93,94,95,
                100,103,105,106,112,113,114,119,122,124,125,131,132,133,
                138,141,143,144,150,151,152,157,160,162,163,169,170,171,
                176,179,181,182,188,189,190,195,198,200,201,207,208,209,
                213,216,218,219,226,227,228,233,236,238,239,245,246,247,
                252,255,257,258,264,265,266,328,330,332,333,340,341,342,
                347,349,351,352,359,360,361,366,369,371,372,378,379,380,
                385,388,390,391,397,398,399,403,406,408,409,416,417,418,
                423,426,428,429,435,436,437,442,445,447,448,454,455,456,
                461,463,465,466,473,474,475,480,483,485,486,492,493,494,
                499,502,504,505,511,512,513,518,521,523,524,530,531,532,
                537,540,542,543,549,550,551,556,558,560,561,568,569,570,
                592,593,594,597,599,600,606,607,608,611,612,613,616,618,619,625,626,627,
                630,631,632,634,636,637,644,645,646,648,649,650,653,655,656,663,664,665,
                666:779,951:1007, 1331:1387, 1578:1653, 2072:2204, 2452:2470, 2490:2508)
...
text(which.max(sil)+1, max(sil), paste("Optimal k =", 19), pos=4, col="red")
#text(which.max(sil)+1, max(sil), paste("Optimal k ="), pos=4, col="red")

dev.off()
cat("Saved silhouette_plot.png\n")

# Silhouette suggests 19. But that's mainly noise
# Elbow method suggests 7
...
#png(filename = "gap_method_optimalk_12.png", width = 800, height = 600, units = "px")
#plot(gap_stat)
...
kmeans_3pc <- kmeans(pca_data_3pc, 12, nstart=100)
...
final_clusters <- kmeans(pca_data_3pc, centers=12, nstart=100)
...
pvar <- summary(pca_res)$importance["Proportion of Variance", 1:3]  # PC1–PC3 [web:2]
pvar_pc <- round(100 * pvar, 1)

pc1_lab <- paste0("PC1: ", pvar_pc[1], "%")
pc2_lab <- paste0("PC2: ", pvar_pc[2], "%")
pc3_lab <- paste0("PC3: ", pvar_pc[3], "%")

# Custom color palette (cluster 6 = dark teal instead of yellow)
custom_colors_12 <- c("#4DAF4A", "#E4C899", "#377EB8", "#E41A1C", 
...
ggplot(pca_df_3d, aes(PC1, PC2, color = Cluster)) +
  ...
  scale_color_manual(values = custom_colors_12) +
  labs(
    title    = "Epitope Clusters (k=12, 3-PC optimized, sil=0.46)",
    subtitle = paste(pc1_lab, "+", pc2_lab, "+", pc3_lab, "improved separation"),
    x = pc1_lab,
    y = pc2_lab
  ) +
...
# Earlier, we only removed:
### Sites 2-4, variants V,L,I,M,F,Y,W
### Site 5, variants C,S,T,V,L,I,M,F,Y,W
### Sites 6-14, variants V,L,I,M,F,Y,W
### Sites 18-30, variants V,L,I,M,F,Y,W
### Sites 32-35, variants D,E,V,L,I,M,F,Y,W
# Numbering in combined_num_ord_normalized is consistent. e.g. we removed site 2
# mutation to F previously. That was line 24. That is gone. However, we need to
# remove surrounding residies. Probably easiest to turn values to NAs, then remove
# with all other NAs, as below.
# Sites 2-14: rows 20:266
# Sites 18-30: rows 384:570
# Sites 32:41: rows 590:779

## AFTER avg_mat_clean is created
## Build antibody × site matrix

# I want to use z-normalized data
target_ids <- c(20:266, 384:570, 590:779)

combined_num_ord_normalized[
  rownames(combined_num_ord_normalized) %in% target_ids,
] <- NA

combined_num_ord_normalized <- combined_num_ord_normalized[
  rowSums(is.na(combined_num_ord_normalized)) != ncol(combined_num_ord_normalized),
  ,
  drop = FALSE
]
...
# As well as removing sites in center of RBD, will also need to remove
# those responsible for ant-myc stickiness. For ease, remove entire sites:
# 2-14
# 18-30
# 32-41


#nums[nums >= 2] <- nums[nums >= 2] + 13 # Skip 2-14 # 1-14 already skipped because all NAs
#nums[nums >= 18] <- nums[nums >= 18] + 13 # Skip 18-30
#nums[nums >= 32] <- nums[nums >= 32] + 10 # Skip 32-41


nums[nums >= 1] <- nums[nums >= 1] + 14
nums[nums >= 18] <- nums[nums >= 18] +13
nums[nums >= 32] <- nums[nums >= 32] + 10 
...
nums[nums >= 132] <- nums[nums >= 132] + 1 # Skip 132
...
###### Determing PCA sites ##########
# Loadings matrix: sites x PCs
reference <- read.csv("reference.csv", stringsAsFactors = FALSE)
loadings <- pca_res$rotation

top_loadings <- function(pc = 1, n = 250) {
  v <- loadings[, pc]
  
  pos_ord <- order(v, decreasing = TRUE)
  neg_ord <- order(v, decreasing = FALSE)
  
  top_positives <- data.frame(
    site = as.integer(rownames(loadings)[pos_ord][v[pos_ord] > 0][1:n]),
    loading = v[pos_ord][v[pos_ord] > 0][1:n]
  )
  
  top_negatives <- data.frame(
    site = as.integer(rownames(loadings)[neg_ord][v[neg_ord] < 0][1:n]),
    loading = v[neg_ord][v[neg_ord] < 0][1:n]
  )
  
  top_positives$Mutation <- reference$Mutation[top_positives$site]
  top_negatives$Mutation <- reference$Mutation[top_negatives$site]
...
pc1 <- top_loadings(1, 250)
pc2 <- top_loadings(2, 250)

top_PC1_positives <- pc1$positives
top_PC1_negatives <- pc1$negatives
top_PC2_positives <- pc2$positives
top_PC2_negatives <- pc2$negatives

write.csv(top_PC1_positives, "top_PC1_positives.csv", row.names = FALSE)
write.csv(top_PC1_negatives, "top_PC1_negatives.csv", row.names = FALSE)
write.csv(top_PC2_positives, "top_PC2_positives.csv", row.names = FALSE)
write.csv(top_PC2_negatives, "top_PC2_negatives.csv", row.names = FALSE)

write.csv(combined_num_normalized, "combined_num_normalized.csv", row.names = FALSE)
